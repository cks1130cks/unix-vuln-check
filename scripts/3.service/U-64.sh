...
# 1. FTP 서비스 상태 확인 (vsftpd, proftpd, pure-ftpd, ftpd 등)
...
for file in "${ftp_files[@]}"; do
    if [ -f "$file" ]; then
        ...
        if grep -Eq "^root$|^root\s" "$file"; then
            echo "    [양호] $file 파일에 root 계정이 포함되어 있어 root 접속이 차단됩니다."
        else
            echo "    [취약] $file 파일에 root 계정이 포함되어 있지 않습니다. root 계정 접속 차단 설정 필요합니다."
            secure=0
        fi
    fi
done
